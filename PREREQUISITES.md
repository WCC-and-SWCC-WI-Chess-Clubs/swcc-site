...
4. Do all the install steps below inside that Ubuntu terminal, and keep the project checked out on the Linux filesystem (e.g. `~/Projects/swcc-website`) rather than under `/mnt/c/...` for much **better performance**.
...
* nginx -- although this is optional, this is super easy in WSL.
...
mkdir Projects
cd Projects
git clone git@github.com:WCC-and-SWCC-WI-Chess-Clubs/swcc.git swcc-website
cd swcc-website
```
...
Now install NGINX, a lightweight web server. 
```bash
sudo apt update 
...
3. In the path selection dialog, navigate to or type the explicit WSL path:  
   `\wsl.localhost\<DistributionName>\home\<username>\Projects\swcc-website`   
